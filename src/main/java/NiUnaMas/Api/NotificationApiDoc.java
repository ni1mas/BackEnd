...
public interface NotificationApiDoc {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 400, message = "Error creating the notification: (concrete error)", response = ApiError.class) })
    @RequestMapping(value = "/users/{id}/notifications/sendNotification/{date}",
    ...
    SuccessfulAction sendNotification(@ApiParam(value = "Notification sent." ,required=true ) @RequestBody Notification notification,
                                                                              ...
                                                                              @ApiParam(value = "The date of the notification") @PathVariable("date") long date);
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Notification created successfully.", response = SuccessfulAction.class),
            @ApiResponse(code = 400, message = "Error creating the notification: (concrete error)", response = ApiError.class) })
    @RequestMapping(value = "/users/{id}/notifications/getNotification",
            produces = { "application/json" },
            method = RequestMethod.GET)
    SuccessfulAction getNotification(@ApiParam(value = "ID of the user.",required=true ) @PathVariable("id") String id);
